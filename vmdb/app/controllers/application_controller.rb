...
class ApplicationController < ActionController::Base
  ...
  if Vmdb::Application.config.action_controller.allow_forgery_protection
    protect_from_forgery :secret => MiqDatabase.first.csrf_secret_token 
  end
  ...
  def error_render_method(e)
    case e
    when ActionController::RoutingError # Don't handle routing errors here
      raise
    else
      ...
      respond_to do |format|
        format.js do
          render :update do |page|    # AJAX, replace the main area with error
            @miq_exception = msg      # Set inside render block, for some reason doesnt work if set it outside
            page.replace_html("exception_div", :partial=>"layouts/exception_contents")
...
#     page.replace("flash_msg_div", :partial=>"layouts/flash_msg")
  ...
  def panel_control
    ...
    render :update do |page|                    # Use JS to update the display
      if @panels[panel]==nil || @panels[panel] == false
        @panels[panel] = true
        page << "$('#{panel}').visualEffect('slide_down', {duration:0.3});"
      else
        @panels[panel] = false
        page << "$('#{panel}').visualEffect('slide_up', {duration:0.3});"
      end
    end
  end
  ...
  def render_zgraph()
    if params[:report]
      ...
      if params[:show_title] && params[:show_title] == "no"
        show_title = false
      else
        show_title = true
      end
      rpt.to_graph(@settings[:display][:reporttheme], show_title, MiqReport.graph_options(params[:width], params[:height]))
    else
      if request.parameters[:controller] == "dashboard" && @sb[:report_result_id] # Check for dashboard results
        rr = MiqReportResult.find(@sb[:report_result_id]) # Get the report results from the
        rpt = rr.report_results
      elsif session[:rpt_task_id]
      ...
      rpt.to_graph(@settings[:display][:reporttheme],false,MiqReport.graph_options(params[:width], params[:height]))
    end
    ...
    # Following line replaced to make charts work in IE after ZiYa version 4.6 installed
#   render :xml => rpt.zgraph.to_xml
    render :xml => rpt.zgraph
  end
...
#  def render_pdf(rr=nil)
  ...
  def render_report_data
    unless params[:task_id]                       # First time thru, kick off the report generate task
      if params[:render_type]
        @sb[:render_type] = params[:render_type].to_sym
        rr = MiqReportResult.find(session[:report_result_id]) # Get report task id from the session
        task_id = rr.async_generate_result(
                    @sb[:render_type],
                    {:userid => session[:userid], :session_id=> request.session_options[:id]}
                  )
        wait_for_task(:task_id=>task_id)
      end
      return
    end
  ...
  def send_report_data
    if @sb[:render_rr_id]
      ...
      send_data(generated_result,
                ...
                :type=>"application/#{@sb[:render_type].to_s}")
    end
  end

  def set_db_type(db_type)
    if ["Host", "Vm", "Storage", "Service"].include?(db_type)
      db = params[:db_type].constantize
    elsif db_type == "Repository"
      db = params[:db_type].constantize
      dbname = "repo"
    elsif db_type == "ems_infra"
...
#   cols_key = @view.scoped_association.nil? ? @view.db.to_sym : (@view.db + "-" + @view.scoped_association).to_sym
  ...
  def wait_for_task(options={})
    ...
    unless params[:task_id]                         # No task id came in, this is a first call
      ...
      session[:async][:params][:task_id] = options[:task_id]  # Add in the task id
    end
    if !params[:task_id] || MiqTask.find(params[:task_id]).state != "Finished"  # Task not done, send down ajax retry
      task_id = params[:task_id] || options[:task_id] || session[:async][:params][:task_id]
    ...
    else                                            # Task done, reinvoke the original method
      @_params.merge!(session[:async][:params])     # Merge in the original parms
      self.send(params[:action])                    # Run the original action (method)
    end
  end
  ...
  def event_logs
    ...
    if @record == nil
      redirect_to :action=>"show_list", :flash_msg=>I18n.t("flash.error_no_longer_exists"), :flash_error=>true
      return
    end
  ...
  def report_only
    ...
    if params[:rr_id]                               # Dashboard widget will send in report result id
      rr = MiqReportResult.find(params[:rr_id].to_i)
    else                                            # Else, find report result in the sandbox
      rr = MiqReportResult.find(@sb[:pages][:rr_id])
    end
    ...
    @report = rr.report
    @html = report_build_html_table(rr.report_results,  # Build the html report
                                    rr.html_rows.join)  #   using all html rows
    if params[:type] == "tabular"
      @graph = nil
    elsif params[:type] == "graph"
      @zgraph = true                                # Show the zgraph in the report
    elsif params[:type] == "hybrid"
      @zgraph = true                                # Show the zgraph in the report
    end
    @ght_type = params[:type]
    @title = @report.title
    render :action=>"show" if request.parameters["controller"] == "report"
    render :action=>"show_report" unless request.parameters["controller"] == "report"
  end
...
#   @gtl_url = "/#{request.parameters["controller"]}/show_statistics/" + @record.id.to_s + "?"#
#    @showtype = "details"#
...
#   @showlinks = false
  ...
  def reload
    @_params[:id] = @sb[:trees][@sb[:active_tree]][:active_node]
    tree_select
  end
  ...
  def build_ae_tree(type, name)
    ...
    if @sb[:active_tree] == :automate_tree && @edit && @edit[:new][:fqname]
    ...
    @sb[:trees][:automate_tree][:active_node] = @temp[:active_node] if @sb[:active_tree] == :automate_tree
    tree_nodes = x_build_tree(@sb[:trees][name])

    # Fill in root node details
    root = tree_nodes.first
    root['text'] = "Datastore"
    root['tooltip'] = "Datastore"
    root['im0'] = root['im1'] = root['im2'] = "folder.png"

    @temp[name] = {"id"=>0, "item"=>tree_nodes}.to_json         # JSON object for tree loading
    @sb[:trees][name][:active_node] ||= tree_nodes.first['id']  # Set active node to root if not set
  end

  def set_summary_pdf_data
    if @display == "download_pdf"
      @report_only = true
      @showtype = "download_pdf"
      @options = Hash.new
      @options[:page_layout] = @settings[:views][:dashboards] == "textual" ? "portrait" : "landscape"
      @options[:page_size] = "US-Letter"
      cls = ui_lookup(:model=>"#{@record.class}")
      @options[:title] = "#{cls} \"#{@record.name}\"".html_safe
      @options[:run_date]  = Time.now.strftime("%m/%d/%y %l:%M %P %Z")
      if @record.kind_of?(Vm) || @record.kind_of?(ResourcePool)
        if @record.kind_of?(Vm)
      ...
      pdf_data = make_pdf(:template=>"/layouts/show_pdf",
                      :stylesheets=>["template_pdf_summary"])

      disable_client_cache

      fname = "#{cls}_#{@record.name}_summary_#{format_timezone(Time.now, Time.zone, "fname")}"
      @display = "main"
      send_data(pdf_data, :filename =>"#{fname}.pdf", :type=>"application/pdf" )
    elsif @display == "summary_only"
      @report_only = true
      @showtype = "summary_only"
      @options = Hash.new
      @options[:page_layout] = @settings[:views][:dashboards] == "textual" ? "portrait" : "landscape"
      @options[:page_size] = "US-Letter"
      @options[:title] = "#{ui_lookup(:model=>"Vm")} \"#{@record.name}\""
      @options[:run_date]  = Time.now.strftime("%m/%d/%y %l:%M %P %Z")
    end
  end
  ...
  def build_config_audit(new, current)
    if request.parameters[:controller] == "ops" && @sb[:active_tab] == "settings_server"
    ...
    new.each_key do |k|
      case k
      when :password2, :verify        # Skip password2, verify fields (confirm password)
      else
        if new[k] != current[k]
          if password_field?(k) # Asterisk out password fields
            msg = msg +  k.to_s + ":[*] to [*]"
          elsif new[k].is_a?(Hash)                          # If the field is a hash,
            (new[k].keys | current[k].keys).each do |hk|    #   process keys of the current and new hashes
              if new[k][hk] != current[k][hk]
                if password_field?(hk) # Asterisk out password fields
                  ...
                  i += 1
                  msg = msg +  hk.to_s + ":[" +
                        (["Hash", "Array"].include?(current[k][hk].class.to_s) ? current[k][hk].inspect : current[k][hk].to_s) +
                        "] to [" +
                        (["Hash", "Array"].include?(new[k][hk].class.to_s) ? new[k][hk].inspect : new[k][hk].to_s) +
                        "]"
                end
              end
            end
            ...
            i += 1
            msg = msg +  k.to_s + ":[" +
                  (["Hash", "Array"].include?(current[k].class.to_s) ? current[k].inspect : current[k].to_s) +
                  "] to [" +
                  (["Hash", "Array"].include?(new[k].class.to_s) ? new[k].inspect : new[k].to_s) +
                  "]"
          end
        end
      end
    ...
    msg = msg + ")"
    audit = {:event=>event, :userid => session[:userid], :message=>msg}
  end
  ...
  def get_time_profiles(obj=nil)
    session[:time_profiles] = Hash.new
    region_id = obj ? obj.region_id : MiqRegion.my_region_number
    TimeProfile.in_region(region_id).all(:conditions=> ["(profile_type = ? or (profile_type = ? and  profile_key = ?))", "global", "user", session[:userid]],
                    :order => "lower(description) ASC").collect{|tp| session[:time_profiles][tp.id] = tp.description}
  end
...
#   Time.zone = ActiveSupport::TimeZone::MAPPING["UTC"] if Time.zone.blank?       #if timezone saved didnt exist in Mapping table
  ...
  def get_reports_menu(group=session[:group], tree_type="reports", mode="menu")
    ...
    @sb[:grp_title] = user.admin_user? ?
      ...
      "#{session[:customer_name]} (#{I18n.t("cell_header.model",:model=>"#{ui_lookup(:model=>"MiqGroup")}: #{user.miq_group.description}")})"
    @data = Array.new
      ...
      @data.each do |r|
        ...
        if title[1].nil?
          ...
          if user.admin_user?
            ...
            if title[1] == "Custom"
              reports.push(r.name) if !reports.include?(r.name) && (r.miq_group && user.miq_group.id == r.miq_group.id)
            else
              reports.push(r.name) unless reports.include?(r.name)
            end
          end
  ...
  def view_to_xml(view, from_idx = 0, to_idx = -1, options={})
    ...
    unless @embedded || @no_checkboxes
      ...
      head.add_element("column", {"type"=>"ro", "width"=>1, "align"=>"center"}) # Checkbox column
    end
    if !["miqaeclass","miqaeinstance"].include?(view.db.downcase)  # donot add listicon for AE class show_list
...
#   cols_key = view.scoped_association.nil? ? view.db.to_sym : (view.db + "-" + view.scoped_association).to_sym
    ...
    view.headers.each_with_index do |h,i|
      ...
      col_width = @settings.fetch_path(:col_widths, cols_key, view.col_order[i]) || col_width # Load saved width, if present
      case column_type(view.db, view.col_order[i])
        when :fixnum, :integer, :Fixnum
          align = "right"
          typ = "ron"
        when :float
          align = "right"
      ...
      new_column = head.add_element("column", {"width"=>"#{col_width}", "sort"=>"str", "type"=>typ, "align"=>align})
#     new_column.add_attribute("format", format) if format != nil # Removed grid formatting in sprint 62, now using view.format method below
#     new_column.add_attribute("type", view.col_order[i] == "name" ? "link" : "ro")
      new_column.text = h
    end

    if @row_button  # Show a button as last col
      head.add_element("column", {"type"=>"ro", "width"=>100, "align"=>"center"}) # Button column
    end

#   head.add_element("settings").add_element("colwidth").text = "%"   # Col widths are percent values

    ai = head.add_element("afterInit")                                # Use to run methods after grid init

#   ai.add_element("call",{"command"=>"loadSizeFromCookie"})          # Reload column sizes from browser cookie

#   ai.add_element("call",{"command"=>"enableHeaderMenu"})            # Column chooser in header

#   sortcall = ai.add_element("call",{"command"=>"setSortImgState"})  # Show sort image in header
#   sortcall.add_element("param").text = "true"
#   sortcall.add_element("param").text = "#{@sortcol + 2}"
#   sortcall.add_element("param").text = "#{@sortdir}"

#   call = ai.add_element("call",{"command"=>"splitAt"})              # Split the grid
#   call.add_element("param").text = "2"

    # Add table elements
    table = view.sub_table ? view.sub_table : view.table
    table.data[from_idx..to_idx].each_with_index do |row,i|
      @id = row['id']
      ...
      view.col_order.each_with_index do |col, col_idx|
        ...
        when "hardware.bitness"
          ...
          if ["miqschedule"].include?(view.db.downcase) # Use scheduled tz for formatting, if configured
  ...
  def create_cols_key(view)
    ...
    if ["ServiceTemplate"].include?(view.db)
      key += ("-" + @sb[:active_tree].to_s) if @sb[:active_tree]
    end
    return key.to_sym
  end
  ...
  def add_flash(msg, level = :info, reset = false)
    reset ? @flash_array = Array.new : @flash_array ||= Array.new # Clear array if reset requested
    @flash_array.push({:message=>msg, :level=>level})

    case level
    when :error
        $log.error("MIQ(#{request.parameters["controller"]}_controller-#{request.parameters["action"]}): " + msg)
    when :warning, :info
        $log.debug("MIQ(#{request.parameters["controller"]}_controller-#{request.parameters["action"]}): " + msg)
      end
  ...
  def check_privileges
    ...
    if !session[:userid] ||                                                                           # User not signed on
      ...
      timed_out = false
      if session[:userid] && Time.now - session[:last_trans_time] > get_vmdb_config[:session][:timeout]
        timed_out = true
      end

      reset_session                                           # Clear the session data

      if request.parameters["controller"] == "report" &&      # Handle link from scheduled report email
          request.parameters["action"] == "show_saved" &&
          request.parameters["id"]
        session[:start_url] = url_for(:controller=>request.parameters["controller"],
                                      :action=>request.parameters["action"],
                                      :id=>request.parameters["id"])
      elsif ["show", "x_show"].include?(request.parameters["action"])
          request.parameters["id"]
        session[:start_url] = url_for(:controller=>request.parameters["controller"],
                                      :action=>request.parameters["action"],
                                      :id=>request.parameters["id"])
      ...
      if request.xml_http_request?  # Is this an Ajax request?
        render :update do |page|    # Yes, send JS down to redirect to login
    ...
    if request.parameters["controller"] != nil  # Make sure controller name came in
      return if request.parameters["action"] == "auth_error"

      # Check for expired license, only allow super admins and only allow support controller actions
      if ! MiqLicense.valid?
        add_flash(I18n.t("flash.license_invalid"), :error)
        if session[:userrole] != 'super_administrator'
          if request.xml_http_request?  # Is this an Ajax request?
            render :update do |page|    # Yes, send JS down to redirect to login
              page.redirect_to :controller=>'dashboard', :action => 'login', :timeout => true
            end
          else                          # Not Ajax, do a simple redirect
            redirect_to :controller=>'dashboard', :action => 'login', :timeout => true
          end
          return
        else
          if !["ops"].include?(request.parameters["controller"])
            redirect_to :controller=>"ops"              # Only ops controller allowed
          end
        end
      end
      auth = true
      if request.parameters["action"] == "button" # If a button was pressed, check auth
        task = request.parameters["pressed"]
        # don't need to check auth for buttons in OPS/AE explorer, if OPS/AE explorer are allowed then buttons on the screen are allowed
        # don't need to check acces on specific buttons, they are allowed if user can get to the screen.
        if !["miq_ae_class","miq_policy","miq_ae_customization","ops"].include?(request.parameters["controller"]) &&
            !["miq_request_reload","saved_delete","miq_report_save","perf_refresh","perf_reload","refresh_log"].include?(task) &&
            !task.starts_with?("custom_") && !task.starts_with?("miq_task_") && !task.starts_with?("tp_")
          unless auth = role_allows(:feature=>task)     # Intentional single = so we can check auth later
            add_flash(I18n.t("flash.user_not_authorized"), :error)
            render :update do |page|                    # Use RJS to update the display
              page.replace(:flash_msg_div, :partial=>"layouts/flash_msg")
            end
      ...
      # Check all other URLs against new RBAC rules
      else
        area = request.parameters["controller"]
        task = request.parameters["action"]
        ident = "#{area}_#{task}"
        if MiqProductFeature.feature_exists?(ident)
          unless auth = role_allows(:feature=>ident, :any=>true)  # Intentional single = so we can check auth later
      ...
      unless auth
        $audit_log.failure("Userid [#{session[:userid]}], Role ID [#{User.current_user.miq_user_role.try(:id)}] attempted to access area [#{area}], type [Action], task [#{task}]")
      end
    end
  ...
  # Common Saved Reports button handler routines follow <<<<<<<<<<<<<<<<
  ...
  # Common timeprofiles button handler routines follow <<<<<<<<<<<<<<<<
...
#             params[:search] ||                          #   search or
    ...
    sortcol_sym = ((association ? association : (dbname == "miqreportresult" && @sb[:active_tree] ? @sb[:active_tree].to_s : dbname) ) + "_sortcol").to_sym # Get symbol for sortcol key
    sortdir_sym = ((association ? association : (dbname == "miqreportresult" && @sb[:active_tree] ? @sb[:active_tree].to_s : dbname) ) + "_sortdir").to_sym # Get symbol for sortdir key
    ...
    if params[:ppsetting]                           # User selected new per page value
      if ["job", "miqtask"].include?(dbname)
        ...
        @settings[:perpage][@gtl_type.to_sym] = params[:ppsetting].to_i # Set the per page setting for this gtl type
      end
    ...
    @items_per_page = ["job", "miqtask"].include?(dbname) ?
      ...
      (request.parameters[:controller].downcase == "miq_policy" ? ONE_MILLION : @settings[:perpage][@gtl_type.to_sym])        # Get per page for this gtl type
    ...
    if session[:menu_click] && session[:sandboxes][params[:sb_controller].to_sym][:chart_reports]
      chart_reports = session[:sandboxes][params[:sb_controller].to_sym][:chart_reports]
...
#     page.visual_effect(:blind_down, "tag_box_div")  if session[:applied_tags] == nil && @applied_tags != nil  # Show div if not shown already
      ...
      if @grid_xml                                  # Replacing a grid
        page << "xml = \"#{escape_javascript(@grid_xml)}\";"            # Set the XML data
  ...
  # Open the SVN entries file to find the build number at line 4
  def getSvnBuild
    build = "NA"
    file = File.join(File.expand_path(Rails.root), ".svn/entries")
    if File.exists?(file)
      data = File.read(file)
      if data.starts_with?("<?xml")
        build = Hash.from_xml(data)["wc_entries"]["entry"][0]["committed_rev"]
      else
        build = data.split[2]
      end
    end
  ...
  def build_created_audit(rec, eh)
    msg = "[#{eh[:new][:name]}] Record created ("
    event = "#{rec.class.to_s.downcase}_record_add"
    i = 0
    eh[:new].each_key do |k|
      unless k.to_s.ends_with?("password2") || k.to_s.ends_with?("verify")
        if password_field?(k) # Asterisk out password fields
          msg = msg +  k.to_s + ":[*] to [*]"
        elsif eh[:new][k].is_a?(Hash)   # If the field is a hash,
          eh[:new][k].each_key do |hk|  #   process all of the keys
            if password_field?(hk) # Asterisk out password fields
              msg = msg +  hk.to_s + ":[*] to [*]"
            else
              msg = msg + ", " if i > 0
              i += 1
              msg = msg +  hk.to_s + ":[" +
                    (["Hash", "Array"].include?(eh[:new][k][hk].class.to_s) ? eh[:new][k][hk].inspect : eh[:new][k][hk].to_s) +
                    "]"
            end
          end
        else
          ...
          i += 1
          msg = msg +  k.to_s + ":[" +
                (["Hash", "Array"].include?(eh[:new][k].class.to_s) ? eh[:new][k].inspect : eh[:new][k].to_s) +
                "]"
        end
      end
    end
    msg = msg + ")"
    audit = {:event=>event, :target_id=>rec.id, :target_class=>rec.class.base_class.name, :userid => session[:userid], :message=>msg}
  end
  ...
  def build_saved_audit(rec, eh)
    msg = "[#{eh[:new][:name] ? eh[:new][:name] : rec[:name]}] Record updated ("
    event = "#{rec.class.to_s.downcase}_record_update"
    i = 0
    eh[:new].each_key do |k|
      unless k.to_s.ends_with?("password2") || k.to_s.ends_with?("verify")
        if eh[:new][k] != eh[:current][k]
          if password_field?(k) # Asterisk out password fields
            msg = msg +  k.to_s + ":[*] to [*]"
          elsif eh[:new][k].is_a?(Hash)                             # If the field is a hash,
            eh[:current][k] = Hash.new if eh[:current][k] == nil    # Make current a blank hash for following comparisons
            (eh[:new][k].keys | eh[:current][k].keys).each do |hk|  #   process keys of the current and new hashes
              if eh[:new][k][hk] != eh[:current][k][hk]
                if password_field?(hk) # Asterisk out password fields
                  msg = msg +  hk.to_s + ":[*] to [*]"
                else
                  msg = msg + ", " if i > 0
                  i += 1
                  msg = msg +  hk.to_s + ":[" +
                        (["Hash", "Array"].include?(eh[:current][k][hk].class.to_s) ? eh[:current][k][hk].inspect : eh[:current][k][hk].to_s) +
                        "] to [" +
                        (["Hash", "Array"].include?(eh[:new][k][hk].class.to_s) ? eh[:new][k][hk].inspect : eh[:new][k][hk].to_s) +
                        "]"
                end
            ...
            i += 1
            msg = msg +  k.to_s + ":[" +
                  (["Hash", "Array"].include?(eh[:current][k].class.to_s) ? eh[:current][k].inspect : eh[:current][k].to_s) +
                  "] to [" +
                  (["Hash", "Array"].include?(eh[:new][k].class.to_s) ? eh[:new][k].inspect : eh[:new][k].to_s) +
                  "]"
          end
  ...
  def prov_redirect(typ=nil)
    if typ  # we need to do this check before doing anything to prevent
      ...
      if VmOrTemplate.includes_template?(vm_ids)
        add_flash(I18n.t("flash.button.task_does_not_apply_to_model", :model=>ui_lookup(:table => "miq_template"), :task=>"Migrate"), :error)
  ...
  def get_global_session_data
    ...
    @sb = session[:sandboxes][request.parameters["controller"].to_sym].blank? ? Hash.new : copy_hash(session[:sandboxes][request.parameters["controller"].to_sym])
    ...
    @server_options = session[:server_options] if ["configuration","support"].include?(request.parameters["controller"])

    # Get timelines hash, if it is in the session for the running controller
    @tl_options = session[(request.parameters["controller"] +"_tl").to_sym]

    session[:adv_search_on] = false if request.parameters["action"][0..3] != "adv_" # Turn off advanced search flag if not an adv search action
    ...
    unless request.xml_http_request?  # Don't capture ajax URLs
      ...
      if request.parameters["action"] == "explorer" # For explorers, don't capture any parms, nil out id
        inbound_url = {
      ...
      when "dashboard", "report", "alert","chargeback"
        session[:tab_url][:vi] = inbound_url if ["show", "show_list", "timeline", "jobs","ui_jobs","usage","chargeback","explorer"].include?(request.parameters["action"])
      when "support"
        session[:tab_url][:set] = inbound_url if ["index"].include?(request.parameters["action"])
      when "configuration","ops","product_update"
        session[:tab_url][:set] = inbound_url if ["explorer","index"].include?(request.parameters["action"])
      when "miq_ae_tools", "miq_ae_class", "miq_ae_customization"
        session[:tab_url][:aut] = inbound_url if ["explorer","resolve","index","explorer","log","import_export","automate_button"].include?(request.parameters["action"])
      when "miq_policy" # Only grab controller and action for policy URLs
        session[:tab_url][:con] = {:controller=>request.parameters["controller"], :action=>request.parameters["action"]} if ["explorer","rsop","export","log"].include?(request.parameters["action"])
      when "miq_capacity"
        session[:tab_url][:opt] = inbound_url if ["utilization","planning","bottlenecks","waste"].include?(request.parameters["action"])
      when "catalog","vm","vm_or_template","miq_request","miq_template","service"
        session[:tab_url][:svc] = inbound_url if ["show", "show_list", "explorer"].include?(request.parameters["action"])
      when "availability_zone","ems_cloud","flavor","security_group","vm_cloud"
        session[:tab_url][:clo] = inbound_url if ["show", "show_list", "explorer"].include?(request.parameters["action"])
      when "ems_cluster","ems_infra","host","miq_request","pxe","repository","resource_pool","storage","vm_infra"
        session[:tab_url][:inf] = inbound_url if ["show", "show_list", "explorer"].include?(request.parameters["action"])
      end
    end
...
#   @edit = session[:edit] ? session[:edit] : nil
  ...
  def get_data_size(data, indent = 0)
    begin
      ...
      data_size = 0
      $log.warn("MIQ(#{request.parameters["controller"]}_controller-#{request.parameters["action"]}): get_data_size error: <#{err}>\n#{err.backtrace.join("\n")}")
    end
    ...
    if indent.zero?
      ...
      if Rails.env.development?
        puts "***** MIQ(#{request.parameters["controller"]}_controller-#{request.parameters["action"]}): #{msg}"
      end
      $log.warn("MIQ(#{request.parameters["controller"]}_controller-#{request.parameters["action"]}): " + msg)
    end
  ...
  def dump_session_data(data, indent = 0)
    begin
      ...
      data_size = 0
      $log.warn("MIQ(#{request.parameters["controller"]}_controller-#{request.parameters["action"]}): dump_session error: <#{err}>\n#{err.backtrace.join("\n")}")
    end

    if indent.zero?
      $log.warn("MIQ(#{request.parameters["controller"]}_controller-#{request.parameters["action"]}): ===============BEGIN SESSION DUMMP===============")
    end
    ...
    if indent.zero?
      $log.warn("MIQ(#{request.parameters["controller"]}_controller-#{request.parameters["action"]}): ===============END SESSION DUMMP===============")
    end
  end
  ...
  def log_data_size(el, value, indent)
    ...
    $log.warn("MIQ(#{request.parameters["controller"]}_controller-#{request.parameters["action"]}): " + line)
  end
  ...
  def set_global_session_data
    ...
    session[:compare_db] = ["MiqTemplate","Vm"].include?(@compare_db.to_s) ? VmOrTemplate : @compare_db if @compare_db != nil
    ...
    session[(request.parameters["controller"] +"_tl").to_sym] = @tl_options if @tl_options != nil
    ...
    unless session[:menu_click]   # Don't save breadcrumbs after a chart menu click
      ...
      when "service"
        session[:tab_bc][:vs] = @breadcrumbs.dup if ["show", "show_list"].include?(request.parameters["action"])
      when "ontap_storage_system","ontap_logical_disk","cim_base_storage_extent","ontap_storage_volume","ontap_file_share","snia_local_file_system","storage_manager"
        session[:tab_bc][:sto] = @breadcrumbs.dup if ["show", "show_list", "index"].include?(request.parameters["action"])
      when "ems_cloud", "availability_zone", "flavor"
        session[:tab_bc][:clo] = @breadcrumbs.dup if ["show", "show_list"].include?(request.parameters["action"])
      when "ems_infra"
        session[:tab_bc][:inf] = @breadcrumbs.dup if ["show", "show_list"].include?(request.parameters["action"])
      when "datacenter"
        session[:tab_bc][:inf] = @breadcrumbs.dup if ["show", "show_list"].include?(request.parameters["action"])
      when "ems_cluster"
        session[:tab_bc][:inf] = @breadcrumbs.dup if ["show", "show_list"].include?(request.parameters["action"])
      when "host"
        session[:tab_bc][:inf] = @breadcrumbs.dup if ["show", "show_list", "log_viewer"].include?(request.parameters["action"])
      when "resource_pool"
        session[:tab_bc][:inf] = @breadcrumbs.dup if ["show", "show_list"].include?(request.parameters["action"])
      when "storage"
        session[:tab_bc][:inf] = @breadcrumbs.dup if ["show", "show_list"].include?(request.parameters["action"])
      ...
      when "vdi_controller","vdi_desktop","vdi_desktop_pool","vdi_endpoint_device","vdi_farm","vdi_user","vm_vdi"
        session[:tab_bc][:vdi] = @breadcrumbs.dup if ["show", "show_list", "index"].include?(request.parameters["action"])
      when "miq_request"
        if @layout == "miq_request_vm"
          session[:tab_bc][:vms] = @breadcrumbs.dup if ["show", "show_list"].include?(request.parameters["action"])
        else
          session[:tab_bc][:inf] = @breadcrumbs.dup if ["show", "show_list"].include?(request.parameters["action"])
        end
      when "vm"
        session[:tab_bc][:vms] = @breadcrumbs.dup if %w{
                                                          ...
                                                          kerneldrivers
                                                          filesystemdrivers
                                                          }.include?(request.parameters["action"])
      end
    end
...
#   end
    ...
    @sb[:tree_hosts] = nil if @sb[:tree_hosts] && (!["ems_folders","descendant_vms"].include?(params[:display]) && !["treesize","tree_autoload"].include?(params[:action]))
    @sb[:tree_vms] = nil if @sb[:tree_vms] && (!["ems_folders","descendant_vms"].include?(params[:display]) && !["treesize","tree_autoload"].include?(params[:action]))

    # Set/clear sandbox (@sb) per controller in the session object
    session[:sandboxes] ||= HashWithIndifferentAccess.new
    session[:sandboxes][request.parameters["controller"].to_sym] = @sb.blank? ? nil : copy_hash(@sb)
    ...
    if session[:sandboxes][:miq_policy]
      session[:sandboxes][:miq_policy][:pi_xml] = nil if session[:sandboxes][:miq_policy][:pi_xml] && (request.parameters[:controller] != "miq_policy" || (request.parameters[:controller] == "miq_policy" && !params[:commit] && !params[:button]))
      session[:sandboxes][:miq_policy][:pi] = nil if session[:sandboxes][:miq_policy][:pi] && (request.parameters[:controller] != "miq_policy" || (request.parameters[:controller] == "miq_policy" && !params[:commit] && !params[:button]))
    end

    # Clearing out session objects that are no longer needed
    session[:myco_tree] = session[:hac_tree] = session[:vat_tree] = nil if request.parameters[:controller] != "ops"
    ...
    session[:network_tree] = nil if !["network"].include?(params[:display]) && params[:action] != "treesize"
    session[:sa_tree] = nil if !["storage_adapters"].include?(params[:display]) && params[:action] != "treesize"
    ...
    session[:report_menu] = session[:report_folders] = session[:menu_roles_tree] = nil if request.parameters[:controller] != "report"
    session[:rsop_tree] = nil  if request.parameters[:controller] != "miq_policy"
  ...
  def visibility_box_edit
    #visibility box
    @edit[:new][:visibility_typ] = params[:visibility_typ] if params[:visibility_typ]
    if @edit[:new][:visibility_typ] == "role"
      @edit[:new][:roles] = Array.new if params[:visibility_typ]    # visibility type changed, reset array
      params.each do |var, val|
        if var.starts_with?("roles_") && val.to_s == "1"
          @edit[:new][:roles].push(var.split("roles_").last) unless @edit[:new][:roles].include?(var.split("roles_").last)
        elsif var.starts_with?("roles_") && val.downcase == "null"
          @edit[:new][:roles].delete(var.split("roles_").last)
        end
      end
      @edit[:new][:roles].sort! unless @edit[:new][:roles].blank?
    elsif @edit[:new][:visibility_typ] == "group"
      @edit[:new][:groups] = Array.new  if params[:visibility_typ]      # visibility type changed, reset array
      params.each do |var, val|
        if var.starts_with?("groups_") && val.to_s == "1"
          @edit[:new][:groups].push(var.split("groups_").last) unless @edit[:new][:groups].include?(var.split("groups_").last)
        elsif var.starts_with?("groups_") && val.downcase == "null"
          @edit[:new][:groups].delete(var.split("groups_").last)
        end
      end
      @edit[:new][:groups].sort! unless @edit[:new][:groups].blank?
    else
      @edit[:new][:roles] ||= Array.new
      @edit[:new][:roles].push("_ALL_") unless @edit[:new][:roles].include?("_ALL_")
    end
  end
